...
> - Built an end-to-end customer churn prediction solution using **Python, SQL, and Power BI**, analyzing 7,000+ telecom customers and identifying **$110K+/month in at-risk revenue**.
> - Trained and compared **Logistic Regression, Decision Tree, and Random Forest** classifiers with a leakage-free scikit-learn pipeline, selecting the final model for **79% recall** on the churn class to align with business retention costs.
> - Designed a **3-page interactive Power BI dashboard** with 11+ DAX measures delivering executive KPIs, a ranked high-risk customer list, and churn-driver analysis used to shape 5 retention recommendations.

## 🎤 Interview Explanation (30–60 seconds)

> "I built a customer churn prediction project using Python, SQL, and Power BI. The goal
> was to predict which telecom customers are likely to leave before they actually do.
> I started by cleaning the dataset — for example, fixing a text-formatted billing column
> and handling blank values for brand-new customers — and then explored churn patterns
> with both pandas and SQL. I trained three models — Logistic Regression, Decision Tree,
> and Random Forest — inside a scikit-learn pipeline to avoid data leakage. Instead of
> picking the model with the best accuracy, I picked the one with the best recall on
> churners, because missing a customer who's about to leave costs far more than sending
> a retention offer to a loyal one. The final model catches about 79% of real churners.
> I then scored every customer with a churn probability and a High/Medium/Low risk level,
> and built a three-page Power BI dashboard: an executive overview with KPIs like churn
> rate and revenue at risk, a ranked call list for the retention team, and an insights
> page showing that contract type, tenure, and payment method drive churn. The main
> recommendation was converting month-to-month customers to annual contracts, since they
> churn at 43% versus 3% for two-year contracts."
